...
const Header = () => {
    ...
    const [currentNav, setCurrentNav] = useState(items[0])

    const RenderNavItems = () => {

        return (
            items.map((item, index) => (
                <Link key={index} href={item.url} onClick={() => setCurrentNav(item)} className={`${currentNav.id == item.id ? "bg-red-700 text-white" : "bg-gray-50 text-gray-600 "} focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-800 border border-white shawdow-md shadow-md cursor-pointer px-3 py-2.5 font-normal text-xs leading-3 rounded`}>{item.name}</Link>
            ))
        )
    }
    ...
    const RenderNavItemsMobile = () => {
        return (
            items.map((item, index) => (
                <Link href={item.url} key={index} onClick={() => { setOpenMobileNav(!openMobileNav); setCurrentNav(item) }} className={`${currentNav.id == item.id ? "hidden" : "block"} px-4 py-3 text-gray-600 bg-gray-50 border border-gray-50 focus:outline-none focus:bg-gray-100 hover:bg-gray-100 duration-100 cursor-pointer text-xs leading-3 font-normal`}>{item.name}</Link>
            ))
        )
    }
            ...
            <div className="bg-white shadow-lg p-3">
                <nav className="flex justify-between">
                    <div className="lg:pr-16 pr-6">
                        <Link onClick={() => { setCurrentNav(items[0]) }} className="flex items-center space-x-3 " href={"/"}>
                ...
                <div className="block md:hidden w-full mt-2 ">
                    <div onClick={() => setOpenMobileNav(!openMobileNav)} className="cursor-pointer p-1 text-white bg-red-700 rounded flex justify-between items-center w-full">
                        <div className="flex space-x-2">
                            <span id="s1" className="font-semibold text-sm leading-3 hidden">Selected: </span><p id="textClicked" className="font-normal text-sm leading-3 focus:outline-none hover:bg-gray-800 duration-100 cursor-pointer ">{currentNav.name ?? "Workspace"}</p>
                        </div>
                    ...
                    <div className="relative">
                        <ul id="list" className={`${openMobileNav ? "h-fit" : "h-0"} overflow-hidden flex transition-all flex-col font-normal text-base leading-4 absolute top-2  w-full rounded shadow-md"`}>
